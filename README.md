...
export class Foo {
	...
	constructor(
		@InjectRepositoryManager('<connectionId>') repoManager
	) {
		repoManager.addMixin({
			name:'ThisIsMyMixin',
...
Check your nestjs startup log. Search for [RepositoryManager]. It will log info on what Repository and Mixins are being used.
